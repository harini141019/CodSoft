...
class GUI():
    def __init__(self,master):
        ...
        self.textfield = Entry(textvariable=self.self.n_username, font='times 15', bd=16, relief='ridge')
        ...
        self.length.grid(row=6, column=1)  

        self.length_textfield = Entry(textvariable=self.n_passwordlen, font='times 15', bd=6, relief='ridge', fg='#DC143C')
        self.length_textfield.grid(row=6, column=0)
        ...
        self.generate = Button(text="GENERATE PASSWORD", bd=3, relief='solid', padx=1, pady=1, font='Veradana", 15 bold', fg='#68228B', bg='#BCEE68', command=self.generate_pass)
        ...
        def generate_pass(self):
            ...
            if length<6:
                messagebox.showerror("Error","Passwoed must be atleast 6 characters long")
        ...
        def accept_field(self):
            with sqlite3.connect("users.db") as db:
                ...
                if cursor.fetchall():
                    messagebox.showerror("This username already existys!","Please use an another username")
                else:
